...
trnalldf = trnalldf[trnalldf.StudyInstanceUID.isin(trndf.StudyInstanceUID)]
...
segls = glob.glob('datamount/segmentations/*.nii')
segls = [s for s in segls if s.split('/')[-1].replace('.nii', '') in trndf.StudyInstanceUID.tolist()]
